...
// Determine data directory (use /tmp in serverless environment like Vercel)
...
// In-memory fallback in case of read-only file systems
let memoryEvaluaciones: any[] = [];
...
memoryEvaluaciones = [...initialSeedEvaluaciones];
  ...
  try {
    if (fs.existsSync(REPO_FILE)) {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        memoryEvaluaciones = parsed;
        return parsed;
      }
    }
  } catch (error) {
    console.warn('Error reading from filesystem, using memory cache:', error);
  }
  return memoryEvaluaciones;
}

function writeEvaluaciones(data: any[]): boolean {
  memoryEvaluaciones = data;
  try {
    fs.writeFileSync(REPO_FILE, JSON.stringify(data, null, 2), 'utf-8');
    return true;
  } catch (error) {
    console.warn('Could not write to disk, saved in memory cache only:', error);
    return true;
  }
}
...
app.get('/api/evaluaciones/stats/summary', (_req, res) => {
  try {
...
app.post('/api/evaluaciones/seed', (_req, res) => {
  try {
...
app.get('/api/evaluaciones/export/csv', (_req, res) => {
  try {
    const records = readEvaluaciones();
    const headers = [
      'ID_EVALUACION',
